...
export default function InputPage() {
	...
	const [selectedPlatforms, setSelectedPlatforms] = useState<Record<keyof UserData, string[]>>({
		code: [],
		socials: [],
		blogs: [],
		contact: [],
	...
	const handleCustomSubmit = () => {
		if (customLink.trim() && customPlatform.trim()) {
			...
			setSelectedPlatforms(prev => ({
				...prev,
				[currentCategory]: [...prev[currentCategory], customPlatform.trim()]
			}))
			setCustomLink("")
			setCustomPlatform("")
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-12">
			<div className="max-w-7xl mx-auto px-6">
				...
				>
					<div className="inline-flex items-center gap-2 bg-gradient-to-r from-teal-500 to-emerald-500 text-white px-4 py-2 mb-2 rounded-full text-sm font-medium">
						<Sparkles className="h-4 w-4" />
						AI-Powered Portfolio Builder
					</div>
					<h1 className="text-4xl lg:text-5xl font-bold bg-gradient-to-r from-teal-600 to-emerald-600 bg-clip-text text-transparent mb-6">
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
					{
						categories.map((category, index) => (
							...
							>
								<Card className="border-gray-200 hover:border-teal-300 transition-all duration-300 hover:shadow-xl group h-full">
									<CardHeader>
									...
									<CardContent className="space-y-4">
												...
												category.options.map((option) => {
													...
													return (
														...
														>
															...
															{
																isConnected && (
																	<CheckCircle className="h-4 w-4 ml-auto text-teal-500" />
																)
															}
														</button>
													)
										...
										<Button
											...
											className="w-full border-dashed border-gray-300 hover:border-teal-400 text-gray-600 hover:text-teal-600"
										>
										...
										<div className="space-y-3">
											{
												userData[category.key].map((item, itemIndex) => (
													<motion.div
														...
														className="flex items-center justify-between p-3 bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg border border-teal-200"
													>
				...
				>
					<Button
						...
						className="bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 text-white text-lg px-12 py-4 h-auto rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
					>
						{
							totalConnections === 0 ? (
